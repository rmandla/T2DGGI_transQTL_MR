...
exp_path = paste0('IGFBP2_',dataset,'_GWAS_harmonized.txt')
outcome_path = paste0('IGFBP2_',dataset,'_harmonized.txt')
...
exp_dat <- read_exposure_data(filename = exp_path,
                              ...
                              eaf_col = 'AF',
                              pval_col = 'P')
exp_dat$Fstat <- (exp_dat$beta.exposure)^2/(exp_dat$se.exposure)^2
#Only keep the variants that are clumped and have Fstat>10
exp_dat = exp_dat[exp_dat$Fstat>=10,]
...
exp_dat_outcome$samplesize.exposure <- N_GWAS
exp_dat_outcome$samplesize.outcome <- N_PQTL

exp_dat_outcome <- steiger_filtering(exp_dat_outcome)
print(exp_dat_outcome)
...
mr_name <- paste0(output_dir, protname, ".",dataset,".steig.fstat.mr.txt")
write.table(mr_results, file=mr_name, sep = '\t', quote = F)

# odds ratio
OR <- generate_odds_ratios(mr_results)
OR_name <- paste0(output_dir, protname, ".",dataset,".steig.fstat.or.txt")
write.table(OR, file=OR_name, sep = '\t', quote = F, row.names = F)

# scatter plot
pdf_name <- paste0(output_dir, protname, ".",dataset,".steig.fstat.scatter.pdf")
...
tryCatch({ 
  ...
  pleio_name <- paste0(output_dir,protname, ".",dataset,".steig.fstat.pleio.txt")
...
tryCatch({ 
  ...
  hetero_name <- paste0(output_dir,protname, ".",dataset,".steig.fstat.hetero.txt")
...
steiger_name <- paste0(output_dir, protname, ".",dataset,".steig.fstat.steiger.txt")
